...
fn empty_content() {
    ...
    assert!(matches!(
        empty_content,
        Err(ParserError::FileContentError(FileContentError::Empty(_)))
    ))
}
...
fn yaml_no_start_with_dagrs() {
    ...
    assert!(matches!(
        forget_dagrs,
        Err(ParserError::YamlTaskError(YamlTaskError::StartWordError))
    ));
}
...
fn yaml_task_no_name() {
    ...
    assert!(matches!(
        no_task_name,
        Err(ParserError::YamlTaskError(YamlTaskError::NoNameAttr(_)))
    ));
}
...
fn yaml_task_not_found_precursor() {
    ...
    assert!(matches!(
        not_found_pre,
        Err(ParserError::YamlTaskError(
            YamlTaskError::NotFoundPrecursor(_)
        ))
    ));
}
...
fn yaml_task_no_script_config() {
    ...
    assert!(matches!(
        script,
        Err(ParserError::YamlTaskError(YamlTaskError::NoScriptAttr(_)))
    ));
}
