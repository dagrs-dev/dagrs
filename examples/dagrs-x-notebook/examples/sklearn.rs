...
use dagrs_x_notebook::{yaml_parser::YamlParser, CommandAction, Parser};
...
impl Action for RootAction {
    async fn run(
        ...
        let mut args = 
        vec![
            "examples/lr_root.py".to_string(),
            format!("{}", data_src),
        ];

        args.append(&mut thetas);

        let cmd_action = CommandAction::new(
            "python",
            args
        );
        let output = cmd_action.run(&mut InChannels::default(), &OutChannels::default(), env).await;
        match output {
            Output::Out(content) => {
                Output::new(content)
            }
...
fn main() {
    let specific_actions: HashMap<String, Box<dyn Action>> = HashMap::from([
        ...
        ("node1".to_string(), Box::new(NodeAction::new(1)) as Box<dyn Action>),
        ("node2".to_string(), Box::new(NodeAction::new(2)) as Box<dyn Action>),
        ("node3".to_string(), Box::new(NodeAction::new(3)) as Box<dyn Action>),
        ("node4".to_string(), Box::new(NodeAction::new(4)) as Box<dyn Action>),
        ("node5".to_string(), Box::new(NodeAction::new(5)) as Box<dyn Action>),
        ("node6".to_string(), Box::new(NodeAction::new(6)) as Box<dyn Action>),
        ("node7".to_string(), Box::new(NodeAction::new(7)) as Box<dyn Action>),
        ("node8".to_string(), Box::new(NodeAction::new(8)) as Box<dyn Action>),
        ("node9".to_string(), Box::new(NodeAction::new(9)) as Box<dyn Action>),
